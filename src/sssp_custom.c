	...
	while(sum!=0) {
		...
		if(vdegree > 1) {
			for(i=0; i<g.nlocalverts; i++) {
				...
				bellman_ford(dist, root); // review - might be called in wrong spot
				...
				num_settled = g.nglobalverts - sum;
				if (num_settled > old_settled) delta = 1.0;			
