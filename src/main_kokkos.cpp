...
//	int rank, size, lgsize;
	...
	Kokkos::View<int64_t*> pred_kokkos("Predecessor view", nlocalverts);
	Kokkos::View<float*> shortest_kokkos("Shortest distance", nlocalverts);
	...
	clean_pred(pred);
	Kokkos::deep_copy(shortest_kokkos, -1);
	Kokkos::deep_copy(pred_kokkos, -1);
setup_sssp_kokkos_globals();
//	run_sssp(bfs_roots[0], &pred[0],shortest); //warm-up
//	run_sssp_openmp(bfs_roots[0], &pred[0],shortest); //warm-up
	run_sssp_kokkos(bfs_roots[0], pred_kokkos, shortest_kokkos); //warm-up
	...
	for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
		...
		clean_pred(&pred[0]);
		clean_shortest(shortest);
		Kokkos::deep_copy(shortest_kokkos, -1.0);
		Kokkos::deep_copy(pred_kokkos, -1);

		/* Do the actual SSSP. */
		double sssp_start = MPI_Wtime();
//		run_sssp(root, &pred[0],shortest);
//		run_sssp_openmp(root, &pred[0],shortest); //warm-up
		run_sssp_kokkos(root, pred_kokkos, shortest_kokkos); //warm-up
...
//			int validation_passed_one = validate_result(0,&tg, nlocalverts, root, pred, shortest,&edge_visit_count);
			int validation_passed_one = validate_result(0,&tg, nlocalverts, root, pred_kokkos.data(), shortest_kokkos.data(),&edge_visit_count);
			double validate_stop = MPI_Wtime();
...
printf("Freed shortest\n");
#endif
	free(bfs_roots);
printf("Freed bfs_roots\n");
	free_graph_data_structure();
printf("Freed graph data structure\n");
